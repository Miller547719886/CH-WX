<template lang="jade">
  .page-edit-address
    group
      x-input(v-bind:field-required="order[0].required" title="姓名" value="" placeholder="请输入您的姓名" v-model="order[0].data" autocomplete="off")
      x-input(v-bind:field-required="order[1].required" title="手机号" value="" placeholder="请输入您的手机号码" v-model="order[1].data" autocomplete="off")
    group
      cell.multilevel-selection(v-bind:field-required="order[2].required" title="所在地区" value="四川省/绵阳市/涪城区/高新街道" v-on:click.native="bindPhone" is-link)
      x-input(v-bind:field-required="order[3].required" title="详细地址" value="" placeholder="请输入详细地址，如XX路XX小区X号" v-model="order[3].data" autocomplete="off")
